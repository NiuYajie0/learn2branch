...
class GAT(K.Model):
    def __init__(self,
                 ...
                 heads, #多头注意力的数量
                 activation,  #激活函数
                 feat_drop,  #输入特征的dropout比例
                 attn_drop,  #注意力特征的dropout比例
...
class GCNPolicy(BaseModel):
    ...
    def __init__(self):
        ...
        self.heads = ([self.num_heads] * self.num_layers) + [self.num_out_heads]
    ...
    def call(self, inputs, training):
        ...
        self.g = dgl.heterograph({('constraint_type', 'edge_type', 'variable_type'): (edge_indices[0],edge_indices[1])})
        # g = dgl.to_bidirected(g)

        self.gat= GAT( self.g,  #DGL的图对象
                 num_layers=self.num_layers,   #层数
                 in_dim=self.emb_size,  #输入特征维度
                 num_hidden=8, #隐藏层特征维度
                 num_classes=64,  #类别数
                 heads=self.heads, #多头注意力的数量
                 activation=tf.nn.elu,  #激活函数
                 feat_drop=0.4,  #输入特征的dropout比例
                 attn_drop=0.4,  #注意力特征的dropout比例
                 negative_slope=0.01, #注意力计算中leaky relu的a值
                 residual=True)
        
        variable_features_1 = self.gat((constraint_features,variable_features))
        
        # OUTPUT
        output = self.output_module(variable_features_1)
