...
def pretrain(model, dataloader, **kwargs):
    ...
    while True:
        for batch in dataloader:
            # 如果 y 里面的某个条目不是 None，就那个条目 to.(device)（放到显卡）
            ...
            g_c, g_ei, g_ev, g_v, g_n_cs, g_n_vs, candss = node_g # candss 是什么来着？
            cand_features, n_cands, best_cands, cand_scores, weights = node_attr

            batched_states = (root_c, root_ei, root_ev, root_v, root_n_cs, root_n_vs, candss, cand_features, None) # 这里加个None是干什么？

            if not model.pre_train(batched_states): # 如果pretrain的时候在模型里面出错或者XXX，就break
                break
...
def process(model, teacher, dataloader, top_k, optimizer=None, **kwargs):
    """
    ...
    device = kwargs['device']
    root_cands_separation = kwargs['root_cands_separation']
    T = kwargs['T']
    alpha = kwargs['alpha']
    ...
    accum_iter = 0
    for batch in dataloader:
        # 现在就是要看一下这些 root_g，node_g，node_attr 都是怎么处理的
        root_g, node_g, node_attr = [map(lambda x:x if x is None else x.to(device) , y) for y in batch]
        ...
        with torch.no_grad():
            if teacher is not None:
                if args.no_e2e: # 这时候应该是用 expert model
...
def exp_main(args):
    ...
    sampleDataRoot = f'data/{research_tag}/{args.trainingSetSize}/samples/'

    valid_data = Dataset(valid_files, sampleDataRoot)
    ...
    pretrain_data = Dataset(pretrain_files, sampleDataRoot)
    ...
    if (
        ...
        teacher = importlib.import_module(f'..models.{teacher_model}.model', __name__)
        teacher = model.GCNPolicy()
        # TODO 应该要能另外指定 teacher_seed
        teacher.restore_state(f"trained_models/{research_tag}/{args.trainingSetSize}/{args.problem}/{teacher_model}/{args.seed}/best_params.pkl")
        teacher.to(device)
        teacher.eval()
    ...
    kwargs['T'] = T
    kwargs['alpha'] = alpha
    kwargs['root_cands_separation'] = root_cands_separation
    ...
    for epoch in range(max_epochs + 1):
        ...
        if (
            ...
            train_data = Dataset(epoch_train_files, sampleDataRoot) # TODO 第二个参数其实好像设成什么都可以
            train_data = torch.utils.data.DataLoader(train_data, batch_size=batch_size,
                                    shuffle = False, num_workers = num_workers, collate_fn = load_batch)
            train_loss, train_kacc = process(model, teacher, train_data, top_k, optimizer, **kwargs)
            log(f"TRAIN LOSS: {train_loss:0.3f} " + "".join([f" acc@{k}: {acc:0.3f}" for k, acc in zip(top_k, train_kacc)]), logfile)

        # TEST
        valid_loss, valid_kacc = process(model, teacher, valid_data, top_k, None, **kwargs)
