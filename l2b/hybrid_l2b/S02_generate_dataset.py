...
def make_samples(in_queue, out_queue, kwargs):
    """
    ...
    node_record_prob = kwargs['node_record_prob']
...
def collect_samples(instances, outdir, rng, n_samples, n_jobs, time_limit, **kwargs):
    """
    Worker loop: fetch an instance, run an episode and record samples.
    ...
    for i in range(n_jobs):
        p = mp.Process(
                target=make_samples,
                args=(orders_queue, answers_queue, kwargs),
                daemon=True)
        workers.append(p)
        p.start()
    ...
    dispatcher = mp.Process(
            target=send_orders,
            args=(orders_queue, instances, rng.randint(2**31), time_limit, tmp_samples_dir, last_episode),
            daemon=True)
    dispatcher.start()
...
def exp_main(args):
    ...
    kwargs = {'node_limit': node_limit, 'node_record_prob': node_record_prob}

    # get instance filenames
    if args.problem == 'setcover':
        instances_train = glob.glob(f'data/{research_tag_instances}/{args.trainingSetSize}/instances/setcover/train_500r_1000c_0.05d/*.lp')
        instances_valid = glob.glob(f'data/{research_tag_instances}/{args.trainingSetSize}/instances/setcover/valid_500r_1000c_0.05d/*.lp')
        instances_test = glob.glob(f'data/{research_tag_instances}/{args.trainingSetSize}/instances/setcover/test_500r_1000c_0.05d/*.lp')
        out_dir = f'data/{research_tag}/{args.trainingSetSize}/samples/setcover/500r_1000c_0.05d/{args.seed}'

    elif args.problem == 'cauctions':
        instances_train = glob.glob(f'data/{research_tag_instances}/{args.trainingSetSize}/instances/cauctions/train_100_500/*.lp')
        instances_valid = glob.glob(f'data/{research_tag_instances}/{args.trainingSetSize}/instances/cauctions/valid_100_500/*.lp')
        instances_test = glob.glob(f'data/{research_tag_instances}/{args.trainingSetSize}/instances/cauctions/test_100_500/*.lp')
        out_dir = f'data/{research_tag}/{args.trainingSetSize}/samples/setcover/500r_1000c_0.05d/{args.seed}'

    elif args.problem == 'indset':
        instances_train = glob.glob(f'data/{research_tag_instances}/{args.trainingSetSize}/instances/indset/train_500_4/*.lp')
        instances_valid = glob.glob(f'data/{research_tag_instances}/{args.trainingSetSize}/instances/indset/valid_500_4/*.lp')
        instances_test = glob.glob(f'data/{research_tag_instances}/{args.trainingSetSize}/instances/indset/test_500_4/*.lp')
        out_dir = f'data/{research_tag}/{args.trainingSetSize}/samples/setcover/500r_1000c_0.05d/{args.seed}'

    elif args.problem == 'facilities':
        instances_train = glob.glob(f'data/{research_tag_instances}/{args.trainingSetSize}/instances/facilities/train_100_100_5/*.lp')
        instances_valid = glob.glob(f'data/{research_tag_instances}/{args.trainingSetSize}/instances/facilities/valid_100_100_5/*.lp')
        instances_test = glob.glob(f'data/{research_tag_instances}/{args.trainingSetSize}/instances/facilities/test_100_100_5/*.lp')
        out_dir = f'data/{research_tag}/{args.trainingSetSize}/samples/setcover/500r_1000c_0.05d/{args.seed}'
        time_limit = 600
    ...
    collect_samples(instances_train, out_dir +"/train", rng, train_size, args.njobs, time_limit, **kwargs)
    print("Success: Train data collection")

    rng = np.random.RandomState(args.seed + 1)
    collect_samples(instances_valid, out_dir +"/valid", rng, valid_size, args.njobs, time_limit, **kwargs)
    print("Success: Valid data collection")

    rng = np.random.RandomState(args.seed + 1)
    collect_samples(instances_test, out_dir +"/test", rng, test_size, args.njobs, time_limit, **kwargs)
    ...
    #     rng = np.random.RandomState(args.seed + 1)
    #     collect_samples(instances_mediumvalid, out_dir +"/mediumvalid", rng, mediumvalid_size, args.njobs, time_limit, **kwargs)
